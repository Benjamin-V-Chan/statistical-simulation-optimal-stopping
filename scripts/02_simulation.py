...
def run_simulations(payoffs, runs, threshold=0.37, seed=None):
    rng = random.Random(seed)
    results = []
    for _ in range(runs):
        shuffled = payoffs[:]
        rng.shuffle(shuffled)
        results.append(simulate_stopping_strategy(shuffled, threshold))
    return results
...
if __name__ == "__main__":
    ...
    results = run_simulations(payoffs, args.runs, args.threshold, args.seed)
